...
class FirebaseRemoteDataSourceImpl implements FirebaseRemoteDataSource {
  ...
  Future<void> createAdoption(AdoptionEntity adoption) async {
    final adoptionCollection =
        firebaseFirestore.collection(FirebaseConsts.adoption);
    ...
    try {
      final adoptinDocRef =
          await adoptionCollection.doc(adoption.adoptionPostId).get();

      if (!adoptinDocRef.exists) {
        adoptionCollection
            .doc(adoption.adoptionPostId)
            .set(newAdoption)
            .then((value) {});
      } else {
  ...
  Stream<List<AdoptionEntity>> readAdoption(AdoptionEntity adoption) {
    ...
    return adoptionCollection.snapshots().map((querySnapshot) =>
        querySnapshot.docs.map((e) => AdoptionModel.fromSnapshot(e)).toList());
  }
  ...
  Stream<List<LostEntity>> readLost(LostEntity lost) {
    ...
    return lostCollection.snapshots().map((querySnapshot) =>
        querySnapshot.docs.map((e) => LostModel.fromSnapshot(e)).toList());
  }
    ...
    return userCollection.snapshots().map(
      (querySnapshot) {
        return querySnapshot.docs
            .where((doc) =>
                doc.id != userId) // Benim UID'me sahip olan dökümanı filtrele
